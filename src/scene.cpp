...
void scene_structure::initialize() {
  ...
  eye1_c.initialize_data_on_gpu(
      mesh_primitive_ellipsoid({0.01f, 0.04f, 0.04f}));
  eye1_c.model.rotation =
      rotation_transform::from_axis_angle({0, 1, 0}, -10 * Pi / 180) *
      rotation_transform::from_axis_angle({0, 0, 1}, 15 * Pi / 180);
  eye2_c.initialize_data_on_gpu(
      mesh_primitive_ellipsoid({0.01f, 0.04f, 0.04f}));
  eye2_c.model.rotation =
      rotation_transform::from_axis_angle({0, 1, 0}, -10 * Pi / 180) *
      rotation_transform::from_axis_angle({0, 0, 1}, -15 * Pi / 180);
  bec_c.initialize_data_on_gpu(mesh_primitive_tetrahedron(
      {0, -0.15f, -0.1f}, {0, 0, 0.05f}, {0.2f, 0, -0.05f}, {0, 0.15f, -0.1f}));
  wing_right_c.initialize_data_on_gpu(
      mesh_primitive_ellipsoid({0.2f, 0.01f, 0.4f}));
  wing_right_c.model.rotation =
      rotation_transform::from_axis_angle({1, 0, 0}, 15 * Pi / 180);
  wing_left_c.initialize_data_on_gpu(
      mesh_primitive_ellipsoid({0.2f, 0.01f, 0.4f}));
  wing_left_c.model.rotation =
      rotation_transform::from_axis_angle({1, 0, 0}, -15 * Pi / 180);
  foot_right_c.initialize_data_on_gpu(mesh_primitive_cubic_grid(
      {0, -0.1f, 0}, {0, 0.1f, 0}, {0, 0.1f, 0.15f}, {0, -0.1f, 0.15f},
      {0.3f, -0.2f, 0}, {0.3f, 0.2f, 0}, {0.3f, 0.2f, 0.05f},
      {0.3f, -0.2f, 0.05f}));
  foot_right_c.model.rotation =
      rotation_transform::from_axis_angle({0, 0, 1}, 25 * Pi / 180);
  foot_left_c.initialize_data_on_gpu(mesh_primitive_cubic_grid(
      {0, -0.1f, 0}, {0, 0.1f, 0}, {0, 0.1f, 0.15f}, {0, -0.1f, 0.15f},
      {0.3f, -0.2f, 0}, {0.3f, 0.2f, 0}, {0.3f, 0.2f, 0.05f},
      {0.3f, -0.2f, 0.05f}));
  foot_left_c.model.rotation =
      rotation_transform::from_axis_angle({0, 0, 1}, -25 * Pi / 180);
  queue_c.initialize_data_on_gpu(mesh_primitive_tetrahedron(
      {0, -0.15f, 0.1f}, {0, 0.15f, 0.1f}, {0, 0, -0.05f}, {-0.2f, 0, 0.05f}));
  queue_c.model.rotation =
      rotation_transform::from_axis_angle({0, 1, 0}, -30 * Pi / 180);

  auto change2grey = {&eye1_c,      &eye2_c,       &bec_c,       &wing_right_c,
                      &wing_left_c, &foot_right_c, &foot_left_c, &queue_c};
  for (auto m : change2grey) m->material.color = {0.3f, 0.3f, 0.3f};
  auto changeNoSpecular = {&body_c,       &head_c,      &bec_c,
                           &wing_right_c, &wing_left_c, &foot_right_c,
                           &foot_left_c};
  for (auto m : changeNoSpecular) m->material.phong.specular = 0;

  body_c.texture.load_and_initialize_texture_2d_on_gpu(
      "../assets/pinguin_cartoon.png");
  head_c.texture.load_and_initialize_texture_2d_on_gpu(
      "../assets/pinguin_cartoon_head2.png");
  ...
  hierarchy.add(wing_right_c, "Wing Right Cartoon", "Body Cartoon",
                {0, 0.53f, -0.05f});
  hierarchy.add(wing_left_c, "Wing Left Cartoon", "Body Cartoon",
                {0, -0.53f, -0.05f});
  hierarchy.add(foot_right_c, "Foot Right Cartoon", "Body Cartoon",
                {0, 0.11f, -0.75f});
  hierarchy.add(foot_left_c, "Foot Left Cartoon", "Body Cartoon",
                {0, -0.11f, -0.75f});
  ...
  // })); body1.model.rotation =
  // rotation_transform::from_axis_angle({0,0,1},-90*Pi/180) *
  // rotation_transform::from_axis_angle({0,1,0},40*Pi/180);
  // body2.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.28f,0.32f,0.25f
  // })); body2.model.rotation =
  // rotation_transform::from_axis_angle({1,0,0},-10*Pi/180);
  // head1.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.25f,0.25f,0.33f
  // })); head1.model.rotation =
  // rotation_transform::from_axis_angle({0,0,1},-90*Pi/180) *
  // rotation_transform::from_axis_angle({0,1,0},10*Pi/180);
  // head2.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.17f,0.19f,0.26f
  // })); head2.model.rotation =
  // rotation_transform::from_axis_angle({0,0,1},-90*Pi/180) *
  // rotation_transform::from_axis_angle({0,1,0},90*Pi/180);
  // eye1.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.01f,0.035f,0.03f
  // })); eye1.model.rotation =
  // rotation_transform::from_axis_angle({0,1,0},-20*Pi/180) *
  // rotation_transform::from_axis_angle({0,0,1},-15*Pi/180);
  // eye2.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.01f,0.035f,0.03f
  // })); eye2.model.rotation =
  // rotation_transform::from_axis_angle({0,1,0},20*Pi/180) *
  // rotation_transform::from_axis_angle({0,0,1},15*Pi/180);
  // bec.initialize_data_on_gpu(mesh_primitive_cubic_grid({ -0.05f,-0.35f,-0.03f
  // }, { 0,-0.3f,0.02f }, { 0.05f,-0.35f,-0.03f }, { 0,-0.4f,-0.08f }, {
  // -0.1f,0,0 }, { 0,0,0.1f }, { 0.1f,0,0 }, { 0,0,-0.1f }));
  // // bec1.initialize_data_on_gpu(my_mesh_primitive_cylinder(0.1f, { 0,0,0 },
  // { 0,-0.2f,0 }, 0.07f));
  // // bec2.initialize_data_on_gpu(my_mesh_primitive_cone(0.1f, 0.2f, { 0,0,0
  // }, { 0,-1,0 }, 0.07f));
  // wing_right.initialize_data_on_gpu(mesh_primitive_ellipsoid({
  // 0.01f,0.1f,0.4f })); wing_right.model.rotation =
  // rotation_transform::from_axis_angle({0,1,0},-15*Pi/180) *
  // rotation_transform::from_axis_angle({1,0,0},30*Pi/180);
  // wing_left.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.01f,0.1f,0.4f
  // })); wing_left.model.rotation =
  // rotation_transform::from_axis_angle({0,1,0},15*Pi/180) *
  // rotation_transform::from_axis_angle({1,0,0},30*Pi/180);
  // queue.initialize_data_on_gpu(mesh_primitive_ellipsoid({ 0.25f,0.2f,0.4f
  // })); queue.model.rotation =
  // rotation_transform::from_axis_angle({1,0,0},70*Pi/180);
  // leg_right.initialize_data_on_gpu(mesh_primitive_cylinder(0.07f, { 0,0,0 },
  // { 0,0,0.2f }, 10, 20, true));
  // leg_left.initialize_data_on_gpu(mesh_primitive_cylinder(0.07f, { 0,0,0 }, {
  // 0,0,0.2f }, 10, 20, true));
  // foot_right.initialize_data_on_gpu(mesh_primitive_cubic_grid({ 0,0,0.06f },
  // { 0.14f,0,0.06f }, { 0.24f,-0.3f,0.03f }, { -0.08f,-0.3f,0.03f }, { 0,0,0
  // }, { 0.14f,0,0 }, { 0.24f,-0.3f,0 }, { -0.08f,-0.3f,0 }));
  // foot_left.initialize_data_on_gpu(mesh_primitive_cubic_grid({ 0,0,0.06f }, {
  // 0.14f,0,0.06f }, { 0.22f,-0.3f,0.03f }, { -0.1f,-0.3f,0.03f }, { 0,0,0 }, {
  // 0.14f,0,0 }, { 0.22f,-0.3f,0 }, { -0.1f,-0.3f,0 }));

  // auto change2grey2 = { &body2, &wing_right, &wing_left, &queue, &leg_right,
  // &leg_left, &foot_right, &foot_left }; for (auto m : change2grey2)
  // 	m->material.color = { 0.3f,0.3f,0.3f };
  // auto change2black = { &eye1, &eye2, &bec };
  // for (auto m : change2black)
  // 	m->material.color = { 0.2f,0.2f,0.2f };
  // auto changeNoSpecular2 = { &body1, &body2, &head1, &head2, &bec,
  // &wing_right, &wing_left, &queue, &leg_right, &leg_left, &foot_right,
  // &foot_left }; for (auto m : changeNoSpecular2)
  ...
  // // hierarchy.add(bec1, "Bec1", "Head2", { 0,-0.1f,0 });
  // // hierarchy.add(bec2, "Bec2", "Bec1", { 0,-0.2f,0 });
