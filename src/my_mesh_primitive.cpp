...
numarray<uint3> connectivity_grid(size_t Nu, size_t Nv) {
...
mesh my_mesh_primitive_cylinder(float radius, vec3 const& p0, vec3 const& p1,
                                float radius2, int Nu, int Nv, bool is_closed) {
  vec3 const p01 = p1 - p0;
  float const L = norm(p01);
  assert_cgp(L > 1e-6f, "Cylinder has 0 length");
  if (radius2 == -1) radius2 = radius;

  vec3 const dir = p01 / L;
  rotation_transform const R =
      rotation_transform::from_vector_transform({0, 0, 1}, dir);
  ...
  for (size_t ku = 0; ku < size_t(Nu); ++ku) {
    for (size_t kv = 0; kv < size_t(Nv); ++kv) {
      ...
      vec3 const q = {radius * std::cos(theta), radius2 * std::sin(theta),
                      L * u};
  ...
  if (is_closed) {
    shape.push_back(my_mesh_primitive_disc(radius, p0, dir, radius2, Nv)
                        .flip_connectivity());
    shape.push_back(my_mesh_primitive_disc(radius, p1, dir, radius2, Nv));
  }
...
// 	return shape;
// }
// mesh mesh_primitive_quadrangle(vec3 const& p00, vec3 const& p10, vec3 const&
// p11, vec3 const& p01)
// {
// 	mesh shape;
...
mesh my_mesh_primitive_disc(float radius, vec3 const& center,
                            vec3 const& normal, float radius2, int N) {
  ...
  rotation_transform const r =
      rotation_transform::from_vector_transform({0, 0, 1}, normal);

  for (size_t k = 0; k < size_t(N); ++k) {
    float const u = k / (N - 1.0f);
    vec3 const p = r * vec3(radius * std::cos(2 * Pi * u),
                            radius2 * std::sin(2 * Pi * u), 0.0f) +
                   center;
    shape.position.push_back(p);
    shape.uv.push_back({float(std::cos(2 * Pi * u)) * 0.5f + 0.5f,
                        float(std::sin(2 * Pi * u)) * 0.5f + 0.5f});
  ...
  for (size_t k = 0; k < size_t(N - 1); ++k)
    shape.connectivity.push_back(
        uint3{unsigned(N), unsigned(k), unsigned(k + 1)});
...
// 			float const v = 1.0f/(Nv+1.0f) * (1-alpha) + alpha*
// Nv/(Nv+1.0f) ;
...
// 	}
// 	for (size_t ku = 0; ku < size_t(Nu-1); ++ku)
// 		shape.connectivity.push_back({Nu*Nv+Nu-1+ku, Nv-1+Nv*(ku+1),
// Nv-1+Nv*ku});
...
// {
// 	assert_cgp(scale.x>0 && scale.y>0 && scale.z>0, "Ellipsoid radius should
// be > 0"); 	assert_cgp(Nu>2 && Nv>2, "Sphere samples should be > 2");
...
// mesh mesh_primitive_grid(vec3 const& p00, vec3 const& p10, vec3 const& p11,
// vec3 const& p01, int Nu, int Nv)
// {
...
// 			vec3 const p = (1-u)*(1-v)*p00 + u*(1-v)*p10 + u*v*p11 +
// (1-u)*v*p01;
...
// mesh mesh_primitive_torus(float r_major, float r_minor, vec3 const& center,
// vec3 const& axis_orientation, int Nu, int Nv)
// {
...
// rotation_transform::from_vector_transform({0,0,1}, axis_orientation);
...
// 			vec3 const p = {
// 				(r_major +
// r_minor*std::cos(theta))*std::cos(phi), 				(r_major
// + r_minor*std::cos(theta))*std::sin(phi),
// r_minor*std::sin(theta)}; 			vec3 const d_theta =
// {-std::sin(theta)*std::cos(phi), -std::sin(theta)*std::sin(phi),
// std::cos(theta)}; 			vec3 const d_phi =
// {-(r_major+r_minor*std::cos(theta))*std::sin(phi),
// (r_major+r_minor*std::cos(theta))*std::cos(phi), 0.0f};
...
mesh my_mesh_primitive_cone(float radius, float height,
                            vec3 const& center_of_base,
                            vec3 const& axis_direction, float radius2,
                            bool is_closed_base, int Nu, int Nv) {
  ...
  mesh shape;
  rotation_transform R =
      rotation_transform::from_vector_transform({0, 0, 1}, axis_direction);
  ...
  for (size_t ku = 0; ku < size_t(Nu); ++ku) {
    for (size_t kv = 0; kv < size_t(Nv); ++kv) {
      ...
      float const r2 = radius2 * (1 - v);

      vec3 const p = R * vec3(r * std::cos(2 * Pi * u),
                              r2 * std::sin(2 * Pi * u), height * v) +
                     center_of_base;
      vec3 const n = R * normalize(vec3{std::cos(2 * Pi * u),
                                        std::sin(2 * Pi * u), radius / height});
      vec2 const uv = {(1 - v) * std::cos(2 * Pi * u) * 0.5f + 0.5f,
                       (1 - v) * std::sin(2 * Pi * u) * 0.5f + 0.5f};
  ...
  for (size_t ku = 0; ku < size_t(Nu - 1); ++ku)
    shape.connectivity.push_back(
        {Nv * ku + Nv - 1, Nv * (ku + 1) + Nv - 1, Nu * Nv + ku});

  if (is_closed_base)
    shape.push_back(my_mesh_primitive_disc(radius, center_of_base,
                                           axis_direction, radius2, Nu)
                        .flip_connectivity());
...
// mesh mesh_primitive_cubic_grid(vec3 const& p000, vec3 const& p100, vec3
// const& p110, vec3 const& p010, vec3 const& p001, vec3 const& p101, vec3
// const& p111, vec3 const& p011, int Nx, int Ny, int Nz)
// {
...
// mesh mesh_primitive_tetrahedron(vec3 const& p0, vec3 const& p1, vec3 const&
// p2, vec3 const& p3)
// {
// 	mesh shape;
...
// mesh mesh_primitive_arrow(vec3 const& p0, vec3 const& p1, float
// cylinder_radius, float cone_length_scale, float cone_radius_scale, int N)
// {
...
// 	assert_cgp(cone_length<norm(p1-p0), "Too short arrow - length of cone >
// length of array"); 	vec3 const u = normalize(p1-p0); 	vec3 const
// p_extremity = p1-u*cone_length;

// 	mesh const cone_extremity = mesh_primitive_cone(cone_radius,
// cone_length, p_extremity, u, true, N); 	mesh const cylinder =
// mesh_primitive_cylinder(cylinder_radius, p0, p_extremity, 2, N, false);
...
// mesh mesh_primitive_frame(frame const& f, float scale, vec3 const& color_ux,
// vec3 const& color_uy, vec3 const& color_uz, vec3 const& color_sphere, float
// frame_thickness)
// {
...
// 	mesh sphere = mesh_primitive_sphere(2.5*frame_thickness*scale, p0,
// 40,20); 	sphere.color.fill(color_sphere);
