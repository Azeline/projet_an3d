...
namespace cgp
{
	...
	mesh my_mesh_primitive_cylinder(float radius, vec3 const& p0, vec3 const& p1, float radius2, int Nu, int Nv, bool is_closed)
	{
		...
		for( size_t ku=0; ku<size_t(Nu); ++ku ) {
			for( size_t kv=0; kv<size_t(Nv); ++kv ) {
				...
				vec3 const q = {radius*std::cos(theta), radius2*std::sin(theta), L*u};
		...
		if(is_closed){
			shape.push_back( my_mesh_primitive_disc(radius, p0, dir, radius2, Nv).flip_connectivity() );
	...
	mesh my_mesh_primitive_disc(float radius, vec3 const& center, vec3 const& normal, float radius2, int N)
	{
		...
		for (size_t k = 0; k < size_t(N); ++k)
		{
			...
			vec3 const p = r * vec3(radius * std::cos(2*Pi*u), radius2 * std::sin(2*Pi*u), 0.0f) + center;
			shape.position.push_back(p);
	...
	mesh my_mesh_primitive_cone(float radius, float height, vec3 const& center_of_base, vec3 const& axis_direction, float radius2, bool is_closed_base, int Nu, int Nv)
	{
		...
		for (size_t ku = 0; ku < size_t(Nu); ++ku)
		{
			for (size_t kv = 0; kv < size_t(Nv); ++kv)
			{
				...
				vec3 const p = R*vec3(r*std::cos(2*Pi*u), r2*std::sin(2*Pi*u), height*v) + center_of_base;
		...
		if(is_closed_base)
			shape.push_back( my_mesh_primitive_disc(radius, center_of_base, axis_direction, radius2, Nu).flip_connectivity() );
